...
set v=111026
set full_name=7z_full.cmd
set log_file=%src_dir:~0,-1%.log
set err_file=%src_dir:~0,-1%.err.log
set exe_dir=.\exe\
call :l_SetDT
set arcfile=%backup_dir%%src_dir:~0,-1%_%dt%_Diff.7z
set remotefile=%src_dir:~0,-1%_%dt%_Diff.7z
set lastarcfile=%backup_dir%%src_dir:~0,-1%_Full_Last.7z

echo ------ >> %log_file%
echo %dt%: 7z DIFF backup started (v.%v%) >> %log_file%
call :l_Run pre_run1 %pre_run1% %pre_run1_parm%
call :l_Run pre_run2 %pre_run2% %pre_run2_parm%

if not exist %lastarcfile% (
	call :l_SetDT
	echo %dt%: full backup doesn't exist, making new one... >> %log_file%
	%full_name%
	exit
)

call :l_SetDT
echo %dt%: %exe_dir%7z.exe u -mx=7 -ssw %lastarcfile% -u- -up0q3r2x2y2z0w2!%arcfile% %src_path%%src_dir% %exclude%>> %log_file%
%exe_dir%7z.exe u -mx=7 -ssw %lastarcfile% -u- -up0q3r2x2y2z0w2!%arcfile% %src_path%%src_dir% %exclude%
if errorlevel 1 (
	call :l_SetDT
...
call :l_SetDT
echo %dt%: DIFF backup completed successfully >> %log_file%
goto :EOF
